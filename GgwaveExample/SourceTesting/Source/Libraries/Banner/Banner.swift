...
open class Banner: UIView {
    ...
    private let imageWidthHeight: CGFloat = 68.0
    ...
    init(title: String, subtitle: String, didTapBlock: @escaping (() -> ())) {
        ...
        imageView.frame = CGRect(x: 0, y: 0, width: imageWidthHeight, height: imageWidthHeight)
...
//        imageView.tintColor = shouldTintImage ? textColor : nil
    ...
    private func configureImageView() {
        ...
        imageView.roundCorners([.topLeft, .topRight, .bottomLeft, .bottomRight], radius: 10)
    }
    ...
    private func initializeSubviews() {
        ...
        let safeAreaTopInsets = UIApplication.shared.delegate?.window??.safeAreaInsets.top ?? 0
        let topEdgeInsets = safeAreaTopInsets + 10
        addConstraints(backgroundView.constraintsEqualToSuperview(UIEdgeInsets(top: topEdgeInsets, left: 16, bottom: 0, right: -16)))
...
//        } else {
            ...
            contentView.addConstraint(imageView.constraintWithAttribute(.centerY, .equal, to: contentView, constant: 0))
            imageView.addConstraint(imageView.constraintWithAttribute(.width, .equal, to: imageWidthHeight))
...
//        }
        ...
        labelView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat("V:|-(0)-[titleLabel]-(8)-[detailLabel]-(5)-|", views: views))
    }
